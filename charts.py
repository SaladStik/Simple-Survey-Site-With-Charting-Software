...
if is_port_in_use(3000):
    ...
    try:
        server_process = subprocess.Popen(['node', 'server.js'], cwd=os.path.dirname(os.path.abspath('server.js')))
...
ip_label = tk.Label(ip_frame, text=f"Everyone please do the survey at: http://{local_ip}:3000 OR scan this QR Code", font=('Helvetica', 36), fg='black', bg='white')
...
qr_img = qr_img.resize((300, 300), Image.LANCZOS)  # Use Image.LANCZOS instead of Image.ANTIALIAS
...
counter_label = tk.Label(root, text=f"Submissions: {count_entries()}", font=('Helvetica', 36), fg='black', bg='white')
